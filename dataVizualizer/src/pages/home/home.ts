...
export class HomePage {
  ...
  //GENERIC
  initProfession(htmlContainer: string,
    htmlSvg: string,
    svgWidth: string,
    svgHeight: string,
    gTitle: string,
    ...
    marginTop: string) {
    //INIT SVG
    this.svg = d3.select(htmlContainer)
      .append(htmlSvg)
      .attr("width", "100%")
      .attr("heigth", "100%")
      .attr("viewBox", "0 0 " + svgWidth + " " + svgHeight);
    this.g = this.svg.append(gTitle)
      .attr("transform", "translate(" + marginLeft + "," + marginTop + ")");

    //INIT AXIS
    this.x = d3Scale.scaleBand().rangeRound([0, +svgWidth]).padding(0.1);
    this.y = d3Scale.scaleLinear().rangeRound([+svgHeight, 0]);
    this.x.domain(StatsBarChart.map((d) => d.musicGenre));
    this.y.domain([0, d3Array.max(StatsBarChart, (d) => d.percentage)]);
